...
TArray<FVector> SphereNormals(const TArray<FVector>& vertices)
{
...
TArray<FVector> SphereAxisPoints(float radius, int32 parallels, float angleStep)
{
    ...
    for (int32 i = 0; i < parallels; ++i)
    {
        ...
        centers.Add(FVector(0.0f, 0.0f, - (unitHeight * radius)));
    }

    return centers;
}

TArray<FVector2D> SphereUVs(int32 pointsPerCircle, int32 parallels)
{
    int32 size = parallels * pointsPerCircle;
    TArray<FVector2D> uvs;
    uvs.SetNum(size);

    float xStep = 1.0 / (pointsPerCircle - 1);
...
void SphereMesh(Mesh& mesh, float radius, int32 parallels, int32 meridians, const FVector& vertexOffset)
{
    ...
    for (const auto& center : SphereAxisPoints(radius, parallels, parallelsAngleStep))
    {
        float localRadius = FMath::Sqrt((radius * radius) - (FMath::Abs(center.Z) * FMath::Abs(center.Z)));
        mesh.Vertices.Append(CircularNGonPoints(meridians, localRadius, center, 0.0f));
    }
    ...
    mesh.Normals.Append(SphereNormals(mesh.Vertices));
    mesh.UVs.Append(SphereUVs(pointsPerCircle, parallels));
}
